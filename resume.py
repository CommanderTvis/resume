...
# ── Styles ──────────────────────────────────────────────────────────────
s_name = ParagraphStyle("Name", fontSize=22, fontName="Helvetica-Bold",
                        textColor=DARK, spaceAfter=2, leading=26)

s_contact = ParagraphStyle("Contact", fontSize=9, fontName="Helvetica",
                           textColor=GRAY, spaceAfter=1, leading=12)

s_summary = ParagraphStyle("Summary", fontSize=10, fontName="Helvetica",
                           textColor=DARK, leading=14, spaceAfter=2,
                           alignment=TA_JUSTIFY)

s_section = ParagraphStyle("Section", fontSize=11, fontName="Helvetica-Bold",
                           textColor=DARK, spaceBefore=7, spaceAfter=3, leading=14)

s_job = ParagraphStyle("Job", fontSize=10, fontName="Helvetica-Bold",
...
s_bullet = ParagraphStyle("Bullet", fontSize=9.5, fontName="Helvetica",
                          textColor=DARK, leading=13, leftIndent=10,
                          spaceAfter=1.5, bulletIndent=0, alignment=TA_JUSTIFY)

s_small = ParagraphStyle("Small", fontSize=9, fontName="Helvetica",
                         textColor=DARK, leading=12, spaceAfter=1)

s_edu = ParagraphStyle("Edu", fontSize=10, fontName="Helvetica-Bold",
                       textColor=DARK, spaceBefore=4, spaceAfter=0, leading=13)

s_edu_meta = ParagraphStyle("EduMeta", fontSize=9, fontName="Helvetica",
                            textColor=GRAY, spaceAfter=1, leading=12)
...
def build():
    doc = SimpleDocTemplate(
        "/home/claude/resume.pdf", pagesize=A4,
    ...
    story.append(KeepTogether([
        Paragraph(f"Software Engineer &mdash; ChromaWay ({lnk('https://chromaway.com/')})", s_job),
        Paragraph("Remote &nbsp;&middot;&nbsp; Apr 2025 &ndash; Present &nbsp;&middot;&nbsp; Full-time (concurrent with online education)", s_meta),
        B(f"Sole maintainer of Rell ({lnk('https://gitlab.com/chromaway/rell')}), "
          "the primary language for the Chromia relational blockchain. "
          "Tree-walk interpreter with SQL generation, ~156K LOC Kotlin."),
        B("New language features: struct.copy() for immutable updates, "
          "try_call_catch with automatic database rollback and typed error results "
          "(try_call_result&lt;T&gt;), @disabled test annotation."),
        B("Migrated build from Maven to Gradle (~22% faster incremental builds). "
        ...
        B("PostgreSQL backend work: query cancellation, migration tooling. "
          "Resolved non-deterministic behaviors across platforms."),
        B("Adopted Kotlin immutable collections throughout the codebase. "
          "Refined the binary compatibility checker."),
        B("Restructured and consolidated Rell language documentation, "
    ...
    story.append(KeepTogether([
        ...
        Paragraph("Yerevan, then Bremen (Germany) &nbsp;&middot;&nbsp; Sep 2022 &ndash; Dec 2024 &nbsp;&middot;&nbsp; Part-time (concurrent with full-time education)", s_meta),
        B("Kotlin/JVM compiler team, working through the K2 compiler transition. "
          "IR backend, performance work, language features."),
        B("Reduced memory allocations in compiler frontend and IR lowering passes. "
          "Bytecode-level optimizations to reduce boxing conversions in generated code."),
        B("Designed and implemented @JvmExposeBoxed annotation for seamless integration "
          "of inline/value classes with Java callers."),
        B("Migrated Android testing infrastructure to the official Android Gradle Plugin framework. "
          "Participated in renovating compiler tests for the new K2 frontend, "
          "including running test suites on Android platform."),
        B("Fixed bugs in IR serialization and lazy declaration deserialization "
          "that caused production compiler crashes."),
    ]))

    # ── MiLaboratories ─────────────────────────────────────────────
    story.append(KeepTogether([
        Paragraph(f"Software Developer (Intern) &mdash; MiLaboratories ({lnk('https://milaboratories.com')})", s_job),
        Paragraph("Remote &nbsp;&middot;&nbsp; Jul &ndash; Aug 2022", s_meta),
        B("Kotlin DSL with statistical and serialization features "
          "for an internal data plotting library (based on lets-plot-kotlin)."),
    ]))

    # ── JetBrains Research ─────────────────────────────────────────
    story.append(KeepTogether([
        Paragraph("Researcher &mdash; JetBrains Research, Nuclear Physics Methods Lab", s_job),
        Paragraph("Dolgoprudny (Russia), then Remote &nbsp;&middot;&nbsp; Sep 2020 &ndash; Apr 2022", s_meta),
        B("The group's research covered non-accelerator particle physics, numerical simulations, "
          "and software development for experimental physics."),
        B(f"Key developer of KMath ({lnk('https://github.com/SciProgCentre/kmath')}, 700+ stars), "
        ...
        B("Cross-platform expression compiler targeting JVM bytecode (ObjectWeb ASM), "
        ...
        B("kmath-gsl: Kotlin/Native bindings to the GNU Scientific Library "
          "via C interop, implementing matrices and vectors. "
          "Native build/toolchain setup, CI, documentation, performance benchmarking."),
        B("KMath-to-LaTeX renderer for computable expressions in Jupyter notebooks."),
    ]))

    # ── Lavrentiev Lyceum ──────────────────────────────────────────
    story.append(KeepTogether([
        Paragraph("Research Intern &mdash; Lab. Math. Modeling, Lavrentiev Lyceum No. 130", s_job),
        Paragraph("Novosibirsk (Russia) &nbsp;&middot;&nbsp; Sep 2019 &ndash; Apr 2021", s_meta),
        B("Interactive system for selecting and studying probability distributions "
          "used in Monte Carlo methods."),
        B("Custom DSL for defining formulas. C++ tree interpreter "
          "and simple bytecode interpreter for performance research."),
        B("HTTP server in Kotlin/Ktor exposing REST API "
        ...
        B("Diploma at the 58th International Scientific Student Conference (MNSK-2020)."),
    ]))
    ...
    skills = [
        ("Languages",
         "Kotlin (expert), Java, C, Python, C#, SQL"),
        ("Compiler Design",
    ...
    story.append(Paragraph("BSc Applied Computer Science &mdash; Constructor University", s_edu))
    story.append(Paragraph("Bremen, Germany &nbsp;&middot;&nbsp; Sep 2023 &ndash; Aug 2027", s_edu_meta))
    ...
    doc.build(story)
    print("Done: /home/claude/resume.pdf")
